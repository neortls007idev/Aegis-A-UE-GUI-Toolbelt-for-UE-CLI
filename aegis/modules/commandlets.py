...
def custom_cmdlets_file(uproject: Path) -> Path:
    """Return path to the custom commandlets list."""

    return uproject.parent / ".aegies" / "commandlets.json"


def load_custom_cmdlets(uproject: Path) -> list[str]:
    """Load custom commandlets for ``uproject``."""

    path = custom_cmdlets_file(uproject)
    if not path.exists():
        return []
    data = json.loads(path.read_text(encoding="utf-8"))
    return list(data.get("commandlets", []))


def add_custom_cmdlet(uproject: Path, name: str) -> list[str]:
    """Append ``name`` to the custom commandlets file for ``uproject``."""

    cmdlets = load_custom_cmdlets(uproject)
    if name not in cmdlets:
        cmdlets.append(name)
        path = custom_cmdlets_file(uproject)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(
            json.dumps({"commandlets": cmdlets}, indent=2), encoding="utf-8"
        )
    return cmdlets
